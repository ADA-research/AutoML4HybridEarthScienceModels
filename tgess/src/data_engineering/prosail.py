...
def create_LUT(parameter_ranges, nlcd_class, n_samples=10000, s2_table_path="../data/raw/", \
               s2_table_filename="S2-SRF_COPE-GSEG-EOPG-TN-15-0007_3.0.xls", out_file="PROSAIL_LUT"):
    """
...
in_situ_df = pd.read_csv("../data/processed/GBOV_RM07_in_situ_train.csv")


def spectral_similarity_rmse(cfg, in_situ_df, target_cols=("LAI_Warren", "lai"), margin=0.1, \
    ...
    #         "lidfb":(-0.15,-0.15),
        ...
        "tto":(0,10),
        "psi":(0,360),
        "rsoil":(cfg["rsoil_min"], cfg["rsoil_max"]),
        "psoil":(cfg["psoil_min"], cfg["psoil_max"]),
        "typelidf":(2,2),     
        "alpha":(40.0,40.0),
    }    
...
def auto_tune_prosail_parameters(n_samples=1000):
    ...
    psoil_min = UniformFloatHyperparameter("psoil_min", 0.0, 1.5, default_value=1.5)
    psoil_max = UniformFloatHyperparameter("psoil_max", 0.0, 1.5, default_value=1.5)
    ...
    scenario = Scenario({"run_obj": "quality",  # we optimize quality (alternative runtime)
                         "runcount-limit": 1,  # max. number of function evaluations; for this example set to a low number
    ...
    smac = SMAC4HPO(scenario=scenario, rng=np.random.RandomState(42),
                    tae_runner=spectral_similarity_rmse, tae_kwargs={"in_situ_df":in_situ_df})
